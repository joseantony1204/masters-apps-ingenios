...
import { Personas, Personasnaturales, Adclientes } from '@/types';
...
export default function Edit({ personas, personasnaturales, clientes, tpidentificacionList, sexosList, ocupacionesList, estadosList}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        id: clientes.id || '',
        personas_id: clientes.persona_id || '',
        referido_id: clientes.referido_id || '',
        comercio_id: clientes.comercio_id || '',
        created_by: clientes.created_by || '',
        updated_by: clientes.updated_by || '',
        deleted_by: clientes.deleted_by || '',
        ...
        estado_id: clientes.estado_id || '', 
      });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('adclientes.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('adclientes.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('adclientes.index')} className="btn btn-outline-secondary">Cancelar</a> 
