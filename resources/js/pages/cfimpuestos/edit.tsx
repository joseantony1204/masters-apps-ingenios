...
export default function Edit({ cfimpuestos}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        ...
        codigo: cfimpuestos.codigo || '',
                             
        nombre: cfimpuestos.nombre || '',
                             
        descripcion: cfimpuestos.descripcion || '',
                             
        valor: cfimpuestos.valor || '',
                             
        activo: cfimpuestos.activo || '',
                             
        tipo_id: cfimpuestos.tipo_id || '',
                             
        created_by: cfimpuestos.created_by || '',
                             
        updated_by: cfimpuestos.updated_by || '',
                             
        deleted_by: cfimpuestos.deleted_by || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('cfimpuestos.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('cfimpuestos.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('cfimpuestos.index')} className="btn btn-outline-secondary">Cancelar</a> 
