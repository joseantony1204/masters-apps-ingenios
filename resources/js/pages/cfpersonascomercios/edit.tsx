...
export default function Edit({ cfpersonascomercios}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        id: cfpersonascomercios.id || '',
        
        activo: cfpersonascomercios.activo || '',
                             
        persona_id: cfpersonascomercios.persona_id || '',
                             
        comercio_id: cfpersonascomercios.comercio_id || '',
                             
        created_by: cfpersonascomercios.created_by || '',
                             
        updated_by: cfpersonascomercios.updated_by || '',
                             
        deleted_by: cfpersonascomercios.deleted_by || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('cfpersonascomercios.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('cfpersonascomercios.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('cfpersonascomercios.index')} className="btn btn-outline-secondary">Cancelar</a> 
