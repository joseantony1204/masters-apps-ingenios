...
export default function Edit({ cfsedes}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        id: cfsedes.id || '',
        
        nombre: cfsedes.nombre || '',
                             
        direccion: cfsedes.direccion || '',
                             
        email: cfsedes.email || '',
                             
        telefono: cfsedes.telefono || '',
                             
        comercio_id: cfsedes.comercio_id || '',
                             
        created_by: cfsedes.created_by || '',
                             
        updated_by: cfsedes.updated_by || '',
                             
        deleted_by: cfsedes.deleted_by || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('cfsedes.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('cfsedes.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('cfsedes.index')} className="btn btn-outline-secondary">Cancelar</a> 
