...
export default function Table({ cfsedes }: Props) {
    ...
    const handleEdit = (id: number) => {
        router.visit(route('cfsedes.edit', id));
    };

    const handleView = (id: number) => {
        router.visit(route('cfsedes.show', id));
    };
                    ...
                    <tr key={cfsedes.id}>
                        ...
                        <ul className="list-inline mb-0">
                            ...
                            */}
                            ...
                            <li className="list-inline-item">
                                <button
                                    onClick={() => handleDelete(cfsedes.id)}
                        ...
                        <td className="text-center">{index + 1}</td>
										...
										<td >{ cfsedes.nombre }</td>
										<td >{ cfsedes.direccion }</td>
										<td >{ cfsedes.email }</td>
										<td >{ cfsedes.telefono }</td>
										<td >{ cfsedes.comercio_id }</td>
										<td >{ cfsedes.created_by }</td>
										<td >{ cfsedes.updated_by }</td>
										<td >{ cfsedes.deleted_by }</td>
