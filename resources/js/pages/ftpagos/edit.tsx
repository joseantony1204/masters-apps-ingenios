...
export default function Edit({ ftpagos}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        id: ftpagos.id || '',
        
        numero: ftpagos.numero || '',
                             
        fecha: ftpagos.fecha || '',
                             
        concepto: ftpagos.concepto || '',
                             
        total: ftpagos.total || '',
                             
        metodo_id: ftpagos.metodo_id || '',
                             
        factura_id: ftpagos.factura_id || '',
                             
        created_by: ftpagos.created_by || '',
                             
        updated_by: ftpagos.updated_by || '',
                             
        deleted_by: ftpagos.deleted_by || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('ftpagos.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('ftpagos.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('ftpagos.index')} className="btn btn-outline-secondary">Cancelar</a> 
