...
export default function Edit({ ftseriales}: Props) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        id: ftseriales.id || '',
        
        serial: ftseriales.serial || '',
                             
        detalle_id: ftseriales.detalle_id || '',
                             
        created_by: ftseriales.created_by || '',
                             
        updated_by: ftseriales.updated_by || '',
                             
        deleted_by: ftseriales.deleted_by || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('ftseriales.update', data.id));
    };

    const handleDelete = () => {
        if (confirm('¿Seguro que quiere eliminar este elemento?')) {
            destroy(route('ftseriales.destroy', data.id));
        }
    };
                            ...
                            <div className="col-12">
                                <div className="row align-items-end justify-content-between g-3">
                                    ...
                                    <div className="col-sm-auto btn-page">
                                        <a href={route('ftseriales.index')} className="btn btn-outline-secondary">Cancelar</a> 
